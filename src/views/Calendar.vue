            ...
            <div class="calendar-item-date" style="position: relative;">
              ...
              <div class="pd-hrzt-3px" v-if="events[item.date.full]">
                <div :key="k" v-for="(ee, k) in eventToArr(events[item.date.full])">
                  <CalendarDate :key="item.date.full+'part1'" :item="item" :ee="ee.part1" :viewEvent="viewEvent" :editEvent="editEvent" :removeEvent="removeEvent" :today="today" />
                  <b-dropdown hoverable aria-role="list" v-if="ee.part2">
                      ...
                      <b-dropdown-item :key="item.date.full+'drop'" aria-role="menu-item" custom class="pd-hrzt-4px pd-vtc-0px">
                        <CalendarDate :key="item.date.full+'part2'"  :item="item" :ee="ee.part2" :viewEvent="viewEvent" :editEvent="editEvent" :removeEvent="removeEvent" :today="today" />
                      </b-dropdown-item>
                  </b-dropdown>
                </div>
              </div>
